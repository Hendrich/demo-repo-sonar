...
export async function getWeatherAnalysis(
  ...
  try {
    ...
    db.all(
      `SELECT * FROM weather_data WHERE city = '${city}'`,
...
export async function exportWeatherData(
  req: Request,
  res: Response
): Promise<void> {
  ...
  try {
    ...
    if (format === "csv") {
      ...
      let csv =
        "id,city,temperature,conditions,humidity,wind_speed,date_recorded\n";
      data.forEach((item) => {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="weather_${city}.csv"`
      );
      res.send(csv);
    } else {
      res.json(data);
